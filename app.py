...
if uploaded_file is not None:
    ...
    with tab1:
        ...
        # 追加情報を表示するかどうか
        show_extra_info = st.checkbox("追加情報を表示", value=False)

        # アイテム表示
        def get_extra_info_html_js(item, sub_df):
            """追加情報の表示／非表示を切り替えるHTMLとJavaScriptを生成"""
        ...
        def render_item_card(idx, row):
            ...
            <div style='background:{bg};border-radius:8px;border:{border};padding:12px;margin-bottom:10px;'>
                ...
                {get_extra_info_html_js(row['item'], sub_df)}
            </div>
            """
            return html
