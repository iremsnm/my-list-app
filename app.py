...
if uploaded_file is not None:
    ...
    def render_card(idx, row):
        _, info_dict, has_info = get_extra_info(row["item"])
        checked = row["checked"]
        card_color = "#e0e0e0" if checked else "#ffffff"

        with st.container():
            col1, col2 = st.columns([3, 7])
            with col1:
                if st.button(f"{idx}. {row['item']}", key=f"card_button_{idx}"):
                    st.session_state.checked[idx - 1] = True
                    st.rerun()
            with col2:
                if show_extra_info and has_info:
                    for k, v in info_dict.items():
                        st.markdown(f"<div style='font-size: 12px; color: {'gray' if checked else 'black'};'>**{k}**: {v}</div>", unsafe_allow_html=True)
